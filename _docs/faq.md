...
!!! info
    Electricity and gas meter entity IDs are now generated by Home Assistant using your configured naming scheme. The device name contains the meter identity and the entity name contains only its existing human-readable, entity-specific name. Existing registered entity IDs are retained automatically, while new or recreated IDs may differ from the historical examples. See [Entity IDs and names](./entity_ids.md) for further details.
