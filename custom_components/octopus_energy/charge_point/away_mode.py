...
class OctopusEnergyChargePointAwayMode(CoordinatorEntity, BaseOctopusEnergyChargePointSensor, BinarySensorEntity, RestoreEntity):
  """Sensor for displaying if away mode is enabled for a charge point."""
  ...
  def _handle_coordinator_update(self) -> None:
    ...
    if (result is not None
      ...
      _LOGGER.debug(f"Updating OctopusEnergyChargePointAwayMode for '{self._charge_point_id}'")
  ...
  async def async_added_to_hass(self):
    ...
    _LOGGER.debug(f'Restored OctopusEnergyChargePointAwayMode state: {self._state}')
