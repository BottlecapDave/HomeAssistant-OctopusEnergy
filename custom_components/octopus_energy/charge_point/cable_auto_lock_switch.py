...
class OctopusEnergyChargePointCableAutoLockSwitch(CoordinatorEntity, BaseOctopusEnergyChargePointSensor, SwitchEntity, RestoreEntity):
  """Switch for turning charge point cable auto lock on and off.

  Only created when the charge point's configuration reports
  isChargeCableAutoLockAvailable == True (same gating as the binary sensor).
  """
  ...
  def _handle_coordinator_update(self) -> None:
    """Determine the current cable auto lock state."""
    ...
    if (result.data is not None
        ...
        and result.data.configuration.isChargeCableAutoLockEnabled is not None):
  ...
  async def async_turn_on(self, **kwargs):
    """Turn on the switch."""
    try:
      await self._client.async_set_charge_point_charge_cable_auto_lock(self._account_id, self._charge_point_id, True)
  ...
  async def async_turn_off(self, **kwargs):
    """Turn off the switch."""
    try:
      await self._client.async_set_charge_point_charge_cable_auto_lock(self._account_id, self._charge_point_id, False)
    except Exception as e:
      if self._is_mocked:
  ...
  async def async_added_to_hass(self):
    ...
    _LOGGER.debug(f'Restored OctopusEnergyChargePointCableAutoLockSwitch state: {self._state}')
