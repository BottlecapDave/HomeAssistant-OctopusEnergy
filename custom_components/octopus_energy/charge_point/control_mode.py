...
class OctopusEnergyChargePointControlMode(CoordinatorEntity, BaseOctopusEnergyChargePointSensor, RestoreSensor):
  """Sensor for displaying the control mode of a charge point."""
  ...
  def _handle_coordinator_update(self) -> None:
    """Retrieve the control mode for the charge point."""
    current = now()
    ...
    if (result is not None
        ...
        and result.data.controlMode is not None):
      _LOGGER.debug(f"Updating OctopusEnergyChargePointControlMode for '{self._charge_point_id}'")
  ...
  async def async_added_to_hass(self):
    ...
    if state is not None and last_sensor_state is not None and self._state is None:
      ...
      _LOGGER.debug(f'Restored OctopusEnergyChargePointControlMode state: {self._state}')
