...
class OctopusEnergyChargePointEcoModeSwitch(CoordinatorEntity, BaseOctopusEnergyChargePointSensor, SwitchEntity, RestoreEntity):
  """Switch for turning charge point eco mode on and off."""
  ...
  def unique_id(self):
    ...
    return f"octopus_energy_charge_point_{self._charge_point_id}_eco_mode_switch"
  ...
  def _handle_coordinator_update(self) -> None:
    """Determine the current eco mode state."""
  ...
  async def async_turn_on(self, **kwargs):
    """Turn on the switch."""
    try:
      await self._client.async_set_charge_point_eco_mode(self._account_id, self._charge_point_id, True)
  ...
  async def async_turn_off(self, **kwargs):
    """Turn off the switch."""
    try:
      await self._client.async_set_charge_point_eco_mode(self._account_id, self._charge_point_id, False)
    except Exception as e:
      if self._is_mocked:
  ...
  async def async_added_to_hass(self):
    ...
    _LOGGER.debug(f'Restored OctopusEnergyChargePointEcoModeSwitch state: {self._state}')
