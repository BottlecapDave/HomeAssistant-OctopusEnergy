...
class OctopusEnergyChargePointRandomDelaySwitch(CoordinatorEntity, BaseOctopusEnergyChargePointSensor, SwitchEntity, RestoreEntity):
  """Switch for turning charge point random delay on and off."""
  ...
  def unique_id(self):
    ...
    return f"octopus_energy_charge_point_{self._charge_point_id}_random_delay_switch"

  @property
  def name(self):
    """Name of the sensor."""
    return f"Random Delay Charge Point ({self._charge_point_id})"
  ...
  def _handle_coordinator_update(self) -> None:
    """Determine the current random delay state."""
    ...
    if (result.data is not None
        ...
        and result.data.configuration.isRandomDelayEnabled is not None):
      self._state = result.data.configuration.isRandomDelayEnabled
  ...
  async def async_turn_on(self, **kwargs):
    """Turn on the switch."""
    try:
      await self._client.async_set_charge_point_random_delay(self._account_id, self._charge_point_id, True)
  ...
  async def async_turn_off(self, **kwargs):
    """Turn off the switch."""
    try:
      await self._client.async_set_charge_point_random_delay(self._account_id, self._charge_point_id, False)
    except Exception as e:
      if self._is_mocked:
  ...
  async def async_added_to_hass(self):
    ...
    _LOGGER.debug(f'Restored OctopusEnergyChargePointRandomDelaySwitch state: {self._state}')
