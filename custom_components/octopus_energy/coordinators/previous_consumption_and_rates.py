    ...
    try:
      if (is_electricity == True):
        ...
        if consumption_data is not None:
          ...
          _LOGGER.debug(f"No previous consumption data retrieved for electricity {identifier}/{serial_number} - start: {captured_consumption_data[0]["start"]}; end: {captured_consumption_data[-1]["end"]}; total: {len(captured_consumption_data)}")
      else:
        ...
        if consumption_data is not None:
          ...
          _LOGGER.debug(f"No previous consumption data retrieved for gas {identifier}/{serial_number} - start: {captured_consumption_data[0]["start"]}; end: {captured_consumption_data[-1]["end"]}; total: {len(captured_consumption_data)}")
