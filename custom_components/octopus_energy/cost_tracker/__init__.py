...
def calculate_consumption(current: datetime, consumption_data: list, new_value: float, old_value: float, is_accumulative_value):
  ...
  if consumption_data is not None and len(consumption_data) > 0 and consumption_data[0]["start"] != start_of_day:
