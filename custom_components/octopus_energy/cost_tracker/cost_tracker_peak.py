...
class OctopusEnergyCostTrackerPeakSensor(CoordinatorEntity, RestoreSensor):
  ...
  def unique_id(self):
    ...
    return f"octopus_energy_cost_tracker_{self._config[CONFIG_COST_NAME]}_peak"
    
  @property
  def name(self):
    """Name of the sensor."""
    return f"Octopus Energy Cost Tracker {self._config[CONFIG_COST_NAME]} (Peak)"
  ...
  async def async_added_to_hass(self):
    ...
    if state is not None and self._state is None:
      ...
      _LOGGER.debug(f'Restored OctopusEnergyCostTrackerPeakSensor state: {self._state}')
  ...
  async def _async_calculate_cost(self, event: EventType[EventStateChangedData]):
    ...
    if (consumption_data is not None and rates_result.rates is not None):
      ...
      if tracked_result is not None and untracked_result is not None:
        self._attributes["total_consumption"] = tracked_result["total_consumption_peak"] + untracked_result["total_consumption_peak"]
        self._state = tracked_result["total_cost_peak"]
