...
class OctopusEnergyCurrentConsumptionDataLastRetrieved(OctopusEnergyBaseDataLastRetrieved):
  ...
  def unique_id(self):
    ...
    return f"octopus_energy_{meter_type}_{self._serial_number}_{self._mpanmprn}_current_consumption_data_last_retrieved"
    
  @property
  def name(self):
    """Name of the sensor."""
    meter_type = "Electricity" if self._is_electricity else "Gas"
    return f"Current Consumption Data Last Retrieved {meter_type} ({self._serial_number}/{self._mpanmprn})"