...
from . import (get_previous_fan_club_discount_information, get_fan_club_number)

_LOGGER = logging.getLogger(__name__)

class OctopusEnergyFanClubPreviousDiscount(CoordinatorEntity, RestoreSensor, OctopusEnergyFanClubSensor):
  """Sensor for displaying the previous discount."""
  ...
  def unique_id(self):
    ...
    return f"octopus_energy_fan_club_{self._account_id}_{get_fan_club_number(self._discount_source)}_previous_discount"
    
  @property
  def name(self):
    """Name of the sensor."""
    return f"Previous Discount Fan Club ({get_fan_club_number(self._discount_source)}/{self._account_id})"
  ...
  def _handle_coordinator_update(self) -> None:
    ...
    if (discounts_result is not None):
      _LOGGER.debug(f"Updating OctopusEnergyFanClubPreviousDiscount for '{self._discount_source}'")
  ...
  async def async_added_to_hass(self):
    ...
    if state is not None and last_sensor_state is not None and self._state is None:
      ...
      _LOGGER.debug(f'Restored OctopusEnergyFanClubPreviousDiscount state: {self._state}')