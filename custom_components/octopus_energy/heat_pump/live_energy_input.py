...
class OctopusEnergyHeatPumpLiveEnergyInput(CoordinatorEntity, BaseOctopusEnergyHeatPumpSensor, RestoreSensor):
  """Sensor for displaying the live energy input of a heat pump."""
  ...
  def unique_id(self):
    ...
    return f"octopus_energy_heat_pump_{self._heat_pump_id}_live_energy_input"

  @property
  def name(self):
    """Name of the sensor."""
    return f"Live Energy Input Heat Pump ({self._heat_pump_id})"
  ...
  def _handle_coordinator_update(self) -> None:
    """Retrieve the live energy input for the heat pump."""
    current = now()
    ...
    if (result is not None 
      ...
      _LOGGER.debug(f"Updating OctopusEnergyHeatPumpLiveEnergyInput for '{self._heat_pump_id}'")

      self._state = float(result.data.heatPumpTimeSeriesPerformance[-1].energyInput.value)
  ...
  async def async_added_to_hass(self):
    ...
    if state is not None and last_sensor_state is not None and self._state is None:
      ...
      _LOGGER.debug(f'Restored OctopusEnergyHeatPumpLiveEnergyInput state: {self._state}')