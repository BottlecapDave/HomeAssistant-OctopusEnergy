...
class OctopusEnergyHeatPumpLiveEnergyOutput(CoordinatorEntity, BaseOctopusEnergyHeatPumpSensor, RestoreSensor):
  """Sensor for displaying the live energy output of a heat pump."""
  ...
  def unique_id(self):
    ...
    return f"octopus_energy_heat_pump_{self._heat_pump_id}_live_energy_output"

  @property
  def name(self):
    """Name of the sensor."""
    return f"Live Energy Output Heat Pump ({self._heat_pump_id})"
  ...
  def _handle_coordinator_update(self) -> None:
    """Retrieve the live energy output for the heat pump."""
    current = now()
    ...
    if (result is not None 
      ...
      _LOGGER.debug(f"Updating OctopusEnergyHeatPumpLiveEnergyOutput for '{self._heat_pump_id}'")

      self._state = float(result.data.heatPumpTimeSeriesPerformance[-1].energyOutput.value)
  ...
  async def async_added_to_hass(self):
    ...
    if state is not None and last_sensor_state is not None and self._state is None:
      ...
      _LOGGER.debug(f'Restored OctopusEnergyHeatPumpLiveEnergyOutput state: {self._state}')