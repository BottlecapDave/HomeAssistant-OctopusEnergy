...
async def async_import_external_statistics_from_consumption(hass: HomeAssistant, unique_id: str, name: str, consumptions, rates, unit_of_measurement: str, consumption_key: str):
  ...
  total_sum = await async_get_last_sum(hass, consumptions[0]["interval_start"], statistic_id)

  peak_statistic_id = f'{statistic_id}_peak'
  peak_sum = await async_get_last_sum(hass, consumptions[0]["interval_start"], peak_statistic_id)

  off_peak_statistic_id = f'{statistic_id}_off_peak'
  off_peak_sum = await async_get_last_sum(hass, consumptions[0]["interval_start"], off_peak_statistic_id)
