...
async def async_load_cached_intelligent_dispatches(hass, account_id: str, device_id: str) -> IntelligentDispatches | None:
  store = storage.Store(hass, "1", f"octopus_energy.intelligent_dispatches_{device_id}")
...
async def async_save_cached_intelligent_dispatches(hass, device_id: str, intelligent_dispatches: IntelligentDispatches):
  if intelligent_dispatches is not None:
    store = storage.Store(hass, "1", f"octopus_energy.intelligent_dispatches_{device_id}")
    await store.async_save(intelligent_dispatches.to_dict())
    _LOGGER.debug(f"Saved intelligent dispatches data for ({device_id})")